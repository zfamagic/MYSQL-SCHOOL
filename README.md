...
## 📖 项目介绍
# 本系统专为高校教学场景设计，采用 B/S 架构，打造简洁美观的响应式界面。系统核心功能包括用户认证、角色权限控制、课程管理、在线选课、成绩管理、考勤记录，支持数据持久化与自动统计。
...
## 💡 注意事项
# 1. 首次运行：务必确保 MySQL 服务已启动，且 .env 中的密码配置正确。
# 2. Git 忽略：项目已配置 .gitignore，自动忽略 node_modules、.env 敏感文件及日志。
# 3. 安全提示：密码采用 bcrypt 加盐加密，JWT Token 有效期为 1 天，请定期更新密钥。